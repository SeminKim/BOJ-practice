### 푼 문제를 모두 적었더니 너무 길어져서 기억나는 문제들만 적어두었다.
...
* 1533: 길의 개수 (21/10/02)
    - 가중치 없는 Adj.matrix를 n제곱한 결과는 각 경로 중 길이 n짜리의 개수를 나타낸다.
...
* 2150: Strongly Connected Component (22/02/28)
    - 타잔 알고리즘을 이용한 SCC를 공부했다. 코사라주에 비해 어려운 것 같다.
...
* 3176: 도로 네트워크 (22/03/26)
    - LCA를 복습하기 좋은 문제. 맞힌 사람 목록을 보면 실행 시간을 더 줄일 수 있을 것 같기도 한데 잘 모르겠다.


* 6549: 히스토그램에서 가장 큰 직사각형 (21/10/25)
    - 꽤 복잡하게 세그먼트 트리로 해결했다. 스택을 활용하여 O(n)에도 풀 수 있다고 한다.
...
* 10999: 구간 합 구하기 2 (22/01/26)
    - Lazy Propagation을 사용하는 법을 배웠다. 어렵다. 비재귀 세그트리에 추가하고 싶어서 끙끙 앓다가 [여기](https://codeforces.com/blog/entry/18051) 를 많이
      참조했다.

* 11266: 단절점 (22/03/25)
    - 단절선을 먼저 풀었음에도 쉽지 않았다.


* 11400: 단절선 (22/02/21)
    - 풀고나니 겨우 이해 되는 것 같은데, 많이 어렵다.
...
* 13409: Black and White Boxes (22/01/05)
    - 옛날에 Hackenbush 관련한 영상을 보고 찜해두었다가 드디어 풀어보았다. 대략 시간복잡도를 맞췄는데도 TLE가 났었는데, 어쩌다 통과하고 나서 다른 풀이들을 보니dictionary를 사용하면 되는
      문제였다.
...
* 15684: 사다리 조작 (22/01/09)
    - 초기 상태가 최대 3개의 가로줄 추가로는 해결할 수 없는 경우를 일찍 분기해주면 실행시간이 아주 많이 줄어드는데, 그냥 테케 저격이 되는 것 같다. diff가 6 이하이면서 가로줄 3개 필요한 경우를 사전
      순으로 가장 늦은 테케를 넣으면 같아질 것이다.
...
* 16235: 나무 재테크 (22/01/12)
    - 시간 제한이 빡빡해서 고생했는데, 별로 좋은 문제라는 느낌은 안든다.
...
* 23291: 어항 정리 (22/02/16)
    - Python의 리스트 슬라이싱을 이용해서 상당히 깔끔하게 구현할 수 있었다.
