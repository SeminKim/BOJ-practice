...
* 10999: 구간 합 구하기 2 (22/01/26)
    - Lazy Propagation을 사용하는 법을 배웠다. 어렵다. 비재귀 세그트리에 추가하고 싶어서
      끙끙 앓다가 [여기](https://codeforces.com/blog/entry/18051) 를 많이 참조했다.
...
* 13409: Black and White Boxes (22/01/05)
    - 옛날에 Hackenbush 관련한 영상을 보고 찜해두었다가 드디어 풀어보았다. 대략 시간복잡도를 맞췄는데도 TLE가 났었는데, 어쩌다 통과하고 나서
      다른 풀이들을 보니dictionary를 사용하면 되는 문제였다.
...
* 15684: 사다리 조작 (22/01/09)
    - 초기 상태가 최대 3개의 가로줄 추가로는 해결할 수 없는 경우를 일찍 분기해주면 실행시간이 아주 많이 줄어드는데, 
      그냥 테케 저격이 되는 것 같다. diff가 6 이하이면서 가로줄 3개 필요한 경우를 사전 순으로 가장 늦은 테케를 넣으면 같아질 것이다. 
